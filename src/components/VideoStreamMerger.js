...
function VideoStreamMerger (opts) {
  ...
  self.width = opts.width || 290
  self.height = opts.height || 218
  self.fps = opts.fps || 25
  self.clearRect = opts.clearRect === undefined ? true : opts.clearRect
  self.videoElm = opts.videoElm || document.createElement('canvas')

  // Hidden canvas element for merging
  self._canvas = self.videoElm
...
VideoStreamMerger.prototype.addStream = function (mediaStream, opts) {
  ...
  if (!videoElement) {
    ...
    // self.videoElm.parentNode.appendChild(self._canvas)
    // if (!stream.mute) {
...
//////////////////
// WEBPACK FOOTER
// ./node_modules/video-stream-merger/src/index.js
// module id = 679
// module chunks = 0
